...
By sourcing ```setup.sh``` the folder ```/pc/choozdsk01/DATA/CAJITA``` will be mounted in ```
data```. In this directory we store additional configurations, geometries and the output of older simulations that can be used when needed.

The geometry argument is optional for building, if not provided the script will ask for it.
